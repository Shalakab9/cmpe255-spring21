...
def main():
    ...
    df_train, df_val, df_test = objcar.validate()
    ...
    del df_val['msrp']
    del df_test['msrp']
    X_train = objcar.prepare_X(df_train)
    w_0, w = objcar.linear_regression(X_train, y_train)

    X_val = objcar.prepare_X(df_val)
    y_pred = w_0 + X_val.dot(w)
    print('validation:', objcar.rmse(y_val, y_pred))

    X_test = objcar.prepare_X(df_test)
    y_pred = w_0 + X_test.dot(w)
    print('test:', objcar.rmse(y_test, y_pred))
